...
## ----import_data,  message = FALSE, warnings = FALSE------------------------------------------------
#import data
...
hm <- hm %>%
  dplyr::mutate(infection = case_when(
    ...
    infection == "uninfected" & MC.Eimeria == "TRUE" ~ "infected_eimeria",
...
test_lab   %>%
  ...
  geom_point(aes(x = predictions, y = WL_max, 
                 color = infection, size = delta_ct_cewe_MminusE)) +
  labs(x = "Predictions: Maximum weight loss", 
       y = "Observed: Maximum weight loss") +
    theme_bw()
